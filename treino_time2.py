...
def load_data(csv_path: Path) -> pd.DataFrame:
    """Load data with proper parsing of quoted space-separated values."""
    ...
    df = pd.DataFrame(data, columns=[
        "TimeStep", "flow_time", "delta_time", "iters_per_timestep",
        "mon_x", "mon_sacarose", "mon_glicose", "mon_etanol"
    ])
...
def forecast_future_steps(
    ...
    with torch.no_grad():
        for future_dt_scaled in future_deltas_scaled:
            ...
            # Update history with new predicted state
            last_row = history[-1].copy()
            last_flow_scaled = last_row[3]
            
            # Reconstruct flow time
...
def main():
    ...
    torch.save(
        {
            ...
            "input_size": 5,
            "hidden_size": 64,
            "num_layers": 2,  # Fixed: Save correct number of layers
        },
        model_path,
    )
    ...
    # Exemplo de uso para passos futuros
